...
public class DiamondMechanicalSieveScreen extends AbstractContainerScreen<DiamondMechanicalSieveMenu> {
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mX, int mY) {
        ...
        int energy = Mth.floor(54.0F * ((float) this.menu.prevEnergy / (float) ExpandereConfig.diamond_mechanical_sieve_energyStorage));
    ...
    public void render(GuiGraphics graphics, int mx, int my, float partialTicks) {
        ...
        if (9 <= rx && rx < 23 && 21 <= ry && ry < 77) {
            MutableComponent energyTooltip = Component.translatable("gui.edexpandere.energy_label").append(Component.translatable("item.edexpandere.energy_display", this.menu.prevEnergy, ExpandereConfig.diamond_mechanical_sieve_energyStorage)).append(" FE");
