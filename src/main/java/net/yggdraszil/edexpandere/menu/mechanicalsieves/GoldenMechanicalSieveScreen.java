...
public class GoldenMechanicalSieveScreen extends AbstractContainerScreen<GoldenMechanicalSieveMenu> {
    ...
    public GoldenMechanicalSieveScreen(GoldenMechanicalSieveMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderBg(GuiGraphics graphics, float partialTick, int mX, int mY) {
        ...
        int energy = Mth.floor(54.0F * ((float) this.menu.prevEnergy / (float) ExpandereConfig.golden_mechanical_sieve_energyStorage));
        graphics.blit(BACKGROUND_TEXTURE, left + 10, top + 22 + 54 - energy, this.imageWidth, 68 - energy, 12, energy);
        int progress = Math.min(21, (int) (((GoldenMechanicalSieveBlockEntity) this.menu.machine).getLogic().getProgress() * 22.0F));
    ...
    public void render(GuiGraphics graphics, int mx, int my, float partialTicks) {
        ...
        if (9 <= rx && rx < 23 && 21 <= ry && ry < 77) {
            MutableComponent energyTooltip = Component.translatable("gui.edexpandere.energy_label").append(Component.translatable("item.edexpandere.energy_display", this.menu.prevEnergy, ExpandereConfig.golden_mechanical_sieve_energyStorage)).append(" FE");
