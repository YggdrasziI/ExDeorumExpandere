...
public class CreativeMechanicalHammerMenu extends AbstractMachineMenu<CreativeMechanicalHammerBlockEntity> {
    private static final ResourceLocation EMPTY_SLOT_HAMMER = new ResourceLocation("exdeorum", "item/empty_slot_hammer");
    private static final int NUM_SLOTS = 3;

    @ObjectHolder(registryName = "minecraft:menu", value = "edexpandere:creative_mechanical_hammer")
    public static final MenuType<CreativeMechanicalHammerMenu> CREATIVE_MECHANICAL_HAMMER = null;

    public CreativeMechanicalHammerMenu(int containerId, Inventory playerInventory, FriendlyByteBuf data) {
        this(containerId, playerInventory, (CreativeMechanicalHammerBlockEntity) readPayload(playerInventory, data));
    }

    public CreativeMechanicalHammerMenu(int containerId, Inventory playerInventory, CreativeMechanicalHammerBlockEntity machine) {
        super(CREATIVE_MECHANICAL_HAMMER, containerId, playerInventory, machine);
    ...
    public ItemStack quickMoveStack(Player player, int clickedSlot) {
        ...
        if (slot.hasItem()) {
            ...
            } else if (CreativeMechanicalHammerBlockEntity.isValidInput(clickedStack)) {
    ...
    private class ProgressDataSlot extends DataSlot {
        @Override
        public int get() {
            return CreativeMechanicalHammerMenu.this.machine.getProgress();
        }

        @Override
        public void set(int value) {
            CreativeMechanicalHammerMenu.this.machine.setProgress(value);
        }
    }
    ...
    private class EnergyDataSlot extends DataSlot {
        @Override
        public int get() {
            return CreativeMechanicalHammerMenu.this.machine.getEnergyStored();
        }

        @Override
        public void set(int value) {
            CreativeMechanicalHammerMenu.this.machine.energy.setStoredEnergy(value);
        }
    }
    ...
    private class RedstoneModeDataSlot extends DataSlot {
        @Override
        public int get() {
            return CreativeMechanicalHammerMenu.this.machine.getRedstoneMode();
        }

        @Override
        public void set(int value) {
            CreativeMechanicalHammerMenu.this.machine.setRedstoneMode(value);
        }
    }
