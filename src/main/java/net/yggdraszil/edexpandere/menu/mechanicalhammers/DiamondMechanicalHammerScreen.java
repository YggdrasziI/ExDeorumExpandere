...
public class DiamondMechanicalHammerScreen extends AbstractContainerScreen<DiamondMechanicalHammerMenu> {
    ...
    protected void renderBg(GuiGraphics graphics, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        //Energy Bar
        int energy = Mth.floor(54.0F * ((float)this.menu.getEnergy() / (float) ExpandereConfig.diamond_mechanical_hammer_energyStorage));
    ...
    public void render(GuiGraphics graphics, int mx, int my, float pPartialTick) {
        ...
        if (9 <= rx && rx < 23 && 14 <= ry && ry < 70) {
            MutableComponent energyTooltip = Component.translatable("gui.edexpandere.energy_label")
                    .append(Component.translatable("item.edexpandere.energy_display", this.menu.getEnergy(), ExpandereConfig.diamond_mechanical_hammer_energyStorage))
                    .append(" FE");
