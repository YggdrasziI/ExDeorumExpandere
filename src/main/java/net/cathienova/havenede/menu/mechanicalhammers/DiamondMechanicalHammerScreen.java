...
import net.cathienova.havenede.block.mechanicalhammers.DiamondMechanicalHammerBlockEntity;
...
public class DiamondMechanicalHammerScreen extends AbstractContainerScreen<DiamondMechanicalHammerMenu> {
    ...
    public DiamondMechanicalHammerScreen(DiamondMechanicalHammerMenu menu, Inventory playerInventory, Component title) {
    ...
    protected void renderBg(GuiGraphics graphics, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        int energy = Mth.floor(54.0F * ((float)this.menu.getEnergy() / (float)HavenConfig.diamond_mechanical_hammer_energyStorage));
        graphics.blit(BACKGROUND_TEXTURE, left + 10, top + 15 + 54 - energy, this.imageWidth, 70 - energy, 12, energy);
        int progress = Mth.ceil(23.0F * ((float)this.menu.getProgress() / (float) DiamondMechanicalHammerBlockEntity.TOTAL_PROGRESS));
    ...
    public void render(GuiGraphics graphics, int mx, int my, float pPartialTick) {
        ...
        if (9 <= rx && rx < 23 && 14 <= ry && ry < 70) {
            MutableComponent energyTooltip = Component.translatable("gui.havenede.energy_label")
                    .append(Component.translatable("item.havenede.energy_display", this.menu.getEnergy(), HavenConfig.diamond_mechanical_hammer_energyStorage))
                    .append(" FE");
