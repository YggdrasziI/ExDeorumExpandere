...
public class NetheriteMechanicalHammerScreen extends AbstractContainerScreen<NetheriteMechanicalHammerMenu> {
    ...
    protected void renderBg(GuiGraphics graphics, float pPartialTick, int pMouseX, int pMouseY) {
        ...
        int energy = Mth.floor(54.0F * ((float)this.menu.getEnergy() / (float)HavenConfig.netherite_mechanical_hammer_energyStorage));
    ...
    public void render(GuiGraphics graphics, int mx, int my, float pPartialTick) {
        ...
        if (9 <= rx && rx < 23 && 14 <= ry && ry < 70) {
            MutableComponent energyTooltip = Component.translatable("gui.havenede.energy_label")
                    .append(Component.translatable("item.havenede.energy_display", this.menu.getEnergy(), HavenConfig.netherite_mechanical_hammer_energyStorage))
                    .append(" FE");
