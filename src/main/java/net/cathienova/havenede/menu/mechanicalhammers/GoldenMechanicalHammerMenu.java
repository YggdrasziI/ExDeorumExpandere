...
import net.cathienova.havenede.block.mechanicalhammers.GoldenMechanicalHammerBlockEntity;
import thedarkcolour.exdeorum.menu.AbstractMachineMenu;
import thedarkcolour.exdeorum.tag.EItemTags;

public class GoldenMechanicalHammerMenu extends AbstractMachineMenu<GoldenMechanicalHammerBlockEntity> {
    private static final ResourceLocation EMPTY_SLOT_HAMMER = new ResourceLocation("exdeorum", "item/empty_slot_hammer");
    private static final int NUM_SLOTS = 3;

    @ObjectHolder(registryName = "minecraft:menu", value = "havenede:golden_mechanical_hammer")
    public static final MenuType<GoldenMechanicalHammerMenu> GOLDEN_MECHANICAL_HAMMER = null;

    public GoldenMechanicalHammerMenu(int containerId, Inventory playerInventory, FriendlyByteBuf data) {
        this(containerId, playerInventory, (GoldenMechanicalHammerBlockEntity) readPayload(playerInventory, data));
    }

    public GoldenMechanicalHammerMenu(int containerId, Inventory playerInventory, GoldenMechanicalHammerBlockEntity machine) {
        super(GOLDEN_MECHANICAL_HAMMER, containerId, playerInventory, machine);
    ...
    public ItemStack quickMoveStack(Player player, int clickedSlot) {
        ...
        if (slot.hasItem()) {
            ...
            } else if (GoldenMechanicalHammerBlockEntity.isValidInput(clickedStack)) {
    ...
    private class ProgressDataSlot extends DataSlot {
        @Override
        public int get() {
            return GoldenMechanicalHammerMenu.this.machine.getProgress();
        }

        @Override
        public void set(int value) {
            GoldenMechanicalHammerMenu.this.machine.setProgress(value);
        }
    }
    ...
    private class EnergyDataSlot extends DataSlot {
        @Override
        public int get() {
            return GoldenMechanicalHammerMenu.this.machine.getEnergyStored();
        }

        @Override
        public void set(int value) {
            GoldenMechanicalHammerMenu.this.machine.energy.setStoredEnergy(value);
        }
    }
    ...
    private class RedstoneModeDataSlot extends DataSlot {
        @Override
        public int get() {
            return GoldenMechanicalHammerMenu.this.machine.getRedstoneMode();
        }

        @Override
        public void set(int value) {
            GoldenMechanicalHammerMenu.this.machine.setRedstoneMode(value);
        }
    }
